package Twinani::Plugin::PageNavigator;
...
sub  register{
  ...
  $app->helper( page_navigator => sub{
      ...
      if( $actual == 1 ){
        ...
        $html .= "<a href=\"" . $self->url_for->query( $param => $actual - 1 , %$params) . "\" class=\"$class\">&lt;&lt;</a>";
      }
      foreach my $number( @ret ){
        ...
        $html .= "<span class=\"$class\">$number</span>";
          ...
          $html .= "<a href=\"" . $self->url_for->query( $param => $number , %$params) ."\" class=\"$class\">$number</a>";
        }
      }
      if( $actual == $count ){
        $html .= "<span class=\"$class\">&gt;&gt;</span>";
